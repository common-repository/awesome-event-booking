...
echo ($region) ? __('Region : ', 'wp_event_booking') . get_the_title($region) : __('Events', 'wp_event_booking');
?></h1>
    </header><!-- .page-header -->
	...
	?>
					      <article id="post-<?php the_ID();?>" <?php post_class();?>>
					      	<header class="event-header">
					      		<?php
	the_title(sprintf('<h2 class="event-title"><a rel="nofollow" href="%s" rel="bookmark">', esc_url(get_permalink())), '</a></h2>');
	?>
					      	</header><!-- .event-header -->
